...
$document = $ruta . 'repositorio/flota/desasignaciones';
$random = md5(rand());
...
$datos = consultaDatosDesasignacion($idDesasig, $rutUsuario);
$datos_check = consultaCheckboxsDesasignacion($idDesasig);
$nombreEmpresa = nombreLogin($_POST['url']);
$idAsig = consultaIdAsigForDesasignacion($patente, $rutPersonal);
				...
				<tr>
					...
					<td style="max-width: 358px; min-width: 358px; width: 358px; ">
						<table style="margin-top: 30px;">
							<TR>
								<td style="padding-left: 0px; font-size: 17px; font-weight: bold; max-width: 358px; min-width: 358px; width: 358px; text-align: center; ">Devolución de Vehículo</td>
							</TR>
						</table>
					</td>
					...
					<td style="width: 100%; max-width: 100%; min-width: 100%; text-align: left;">
						...
						<table style="width: 100%; max-width: 100%; min-width: 100%;">
							<tr>
								...
								<td style="width: 10%; max-width: 10%; min-width: 10%; text-align: left;">
									<?php
										echo $datos[0]['IDPATENTE_DESASIGNACIONES'];
									?>
								</td>
							</tr>
						</table>
							...
							for($i = 0; $i < count($datos_check); $i=$i+3){
								if($i < count($datos_check)){
									...
									echo '
                                        ...
                                        if ($datos_check[$i]["DIFERENCIA"] === 'D' && $datos_check[$i]["DESCONTABLE"] === 'Si' && $datos_check[$i]["DATO"] === 'No'){
                                            echo '' . $datos_check[$i]["ITEM"] . ' <span style="color: red;">Dif. Desc.</span></td>';
                                        }
										else if($datos_check[$i]["DESCONTABLE"] === 'Si' && $datos_check[$i]["DATO"] === 'No'){
											echo '' . $datos_check[$i]["ITEM"] . ' <span style="color: red;">Desc. </span></td>';
										}
										else if(($datos_check[$i]["DIFERENCIA"] === 'D')){
											echo '' . $datos_check[$i]["ITEM"] . ' <span style="color: red;">Dif. </span></td>';
										}
                                        else{
                                            echo '' . $datos_check[$i]["ITEM"] . '</td>';
                                        }
                                }
								if($i+1 < count($datos_check)){
									...
									echo '
										...
										<td style="width: 30%; max-width: 30%; min-width: 30%; text-align: left;">';
										if ($datos_check[$i+1]["DIFERENCIA"] === 'D' && $datos_check[$i+1]["DESCONTABLE"] === 'Si' && $datos_check[$i+1]["DATO"] === 'No'){
                                            echo '' . $datos_check[$i+1]["ITEM"] . ' <span style="color: red;">Dif. Desc.</span></td>';
                                        }
										else if($datos_check[$i+1]["DESCONTABLE"] === 'Si' && $datos_check[$i+1]["DATO"] === 'No'){
											echo '' . $datos_check[$i+1]["ITEM"] . ' <span style="color: red;">Desc. </span></td>';
										}
										else if(($datos_check[$i+1]["DIFERENCIA"] === 'D')){
											echo '' . $datos_check[$i+1]["ITEM"] . ' <span style="color: red;">Dif. </span></td>';
										}
                                        else{
                                            echo '' . $datos_check[$i+1]["ITEM"] . '</td>';
                                        }
								}
								if($i+2 < count($datos_check)){
									...
									echo '
										...
										<td style="width: 30%; max-width: 30%; min-width: 30%; text-align: left;">';
										if ($datos_check[$i+2]["DIFERENCIA"] === 'D' && $datos_check[$i+2]["DESCONTABLE"] === 'Si' && $datos_check[$i+2]["DATO"] === 'No'){
                                            echo '' . $datos_check[$i+2]["ITEM"] . ' <span style="color: red;">Dif. Desc.</span></td>';
                                        }
										else if($datos_check[$i+2]["DESCONTABLE"] === 'Si' && $datos_check[$i+2]["DATO"] === 'No'){
											echo '' . $datos_check[$i+2]["ITEM"] . ' <span style="color: red;">Desc. </span></td>';
										}
										else if(($datos_check[$i+2]["DIFERENCIA"] === 'D')){
											echo '' . $datos_check[$i+2]["ITEM"] . ' <span style="color: red;">Dif. </span></td>';
										}
                                        else{
                                            echo '' . $datos_check[$i+2]["ITEM"] . '</td>';
                                        }
								}
								echo '</tr>
									</table>';
...
actualizarDesasignacionChecklist($idDesasig, $pdfChecklist);
