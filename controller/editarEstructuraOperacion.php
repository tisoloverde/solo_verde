<?php
    ...
    if(count($_POST) > 0){
        ...
        $checkEstructuraOperacion = checkEstructuraOperacionEditar($idestructura);
        if($checkEstructuraOperacion["existe"] > 0){
            $row = actualizaEstructuraOperacion($idCliente, $idServicio, $idActividad, $nomenclatura, $idsubcontratista, $nombre, $idarea, $idestructura, $rutUsuario);
            if($row != 'Error'){
                echo "OK";
            }else{
                echo "Sin datos";
