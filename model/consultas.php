	...
	function consultaPersonalDetalles($rut) {
		...
		if ($con != "No conectado") {
			$sql = "SELECT
				...
				EO.NOMENCLATURA as idCeco
			FROM PERSONAL P
			LEFT JOIN ACT A ON A.IDPERSONAL = P.IDPERSONAL
			LEFT JOIN ESTRUCTURA_OPERACION EO ON EO.IDESTRUCTURA_OPERACION = A.IDCENTRO_COSTO
...
WHERE U.RUT = '{$rutUser}'";
	...
	function consultaListaACTHistorialCOUNT($idEstructuraOperacion, $fechaIni, $fechaFin, $search) {
		...
		if ($con != "No conectado") {
			...
			LEFT JOIN ESTRUCTURA_OPERACION EO ON AH.IDESTRUCTURA_OPERACION = EO.IDESTRUCTURA_OPERACION
			LEFT JOIN PROCESOS_PERIODO PP ON EO.DEFINICION = PP.CECO
			WHERE PP.CECO IS NOT NULL
			AND (PP.FECHAPROC BETWEEN '$fechaIni' AND '$fechaFin')";
